...
exports.DEFAULT_CONFIG = {
    ...
    sbom_paths: ['sbom.cdx.json', 'sbom.spdx.json'],
    provenance_paths: ['provenance.json', '.provenance.json', 'attestation.json'],
};
//# sourceMappingURL=defaults.js.map
...
/**
 * Compliance checks for SBOM and provenance
 */
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.checkSbomPresentAndValid = checkSbomPresentAndValid;
exports.checkSbom = checkSbom;
exports.checkProvenancePresentAndValid = checkProvenancePresentAndValid;
exports.checkProvenance = checkProvenance;
exports.runChecks = runChecks;
const fs_1 = __nccwpck_require__(9896);
/**
 * Detects SBOM format using simple heuristics
 */
function detectSbomFormat(sbomData) {
    if (typeof sbomData !== 'object' || sbomData === null) {
        return 'unknown';
    }
    const data = sbomData;
    // CycloneDX: has bomFormat AND specVersion field (both required per spec)
    if ('bomFormat' in data && 'specVersion' in data) {
        return 'CycloneDX';
    }
    // SPDX: has spdxVersion and SPDXID fields
    if ('spdxVersion' in data && 'SPDXID' in data) {
        return 'SPDX';
    }
    return 'unknown';
}
/**
 * Extract key fields from SBOM for evidence
 */
function extractSbomFields(sbomData, format) {
    if (typeof sbomData !== 'object' || sbomData === null) {
        return '';
    }
    const data = sbomData;
    ...
    if (format === 'CycloneDX') {
        if ('bomFormat' in data)
            fields.push(`bomFormat: ${data.bomFormat}`);
        if ('specVersion' in data)
            fields.push(`specVersion: ${data.specVersion}`);
        if ('version' in data)
            fields.push(`version: ${data.version}`);
    }
    else if (format === 'SPDX') {
        if ('spdxVersion' in data)
            fields.push(`spdxVersion: ${data.spdxVersion}`);
        if ('SPDXID' in data)
            fields.push(`SPDXID: ${data.SPDXID}`);
        if ('name' in data)
...
/**
 * Check if SBOM is present and valid (JSON + format detection)
 */
function checkSbomPresentAndValid(config, aggregator) {
    if (!config.require_sbom) {
        return;
    }
    const missingPaths = [];
    let foundPath = null;
    let sbomData = null;
    let format = 'unknown';
    // Check if any SBOM file exists
    for (const path of config.sbom_paths) {
    ...
    // If no SBOM found, report violation
    if (!foundPath) {
        const evidence = missingPaths.map(path => ({
            type: 'missing_file',
            value: path,
        }));
        aggregator.add({
            code: 'SBOM_PRESENT_AND_VALID',
            title: 'SBOM not found',
            severity: 'error',
            details: `No SBOM files were found at the configured paths: ${missingPaths.join(', ')}`,
            evidence,
            resolution: 'Generate an SBOM file using tools like CycloneDX or SPDX and place it at one of the configured paths.',
        });
        return;
    }
    // Try to parse JSON
    try {
        const fileContent = (0, fs_1.readFileSync)(foundPath, 'utf-8');
        sbomData = JSON.parse(fileContent);
    }
    catch (error) {
        const err = error;
        const errorSnippet = err.message.substring(0, 200);
        const evidence = [
            {
                type: 'file_path',
                value: foundPath,
        ...
        aggregator.add({
            code: 'SBOM_PRESENT_AND_VALID',
            title: 'SBOM file has invalid JSON',
            severity: 'error',
            details: `SBOM file at ${foundPath} contains invalid JSON: ${errorSnippet}`,
            evidence,
            resolution: 'Ensure the SBOM file is valid JSON format.',
        });
        return;
    }
    // Detect format
    format = detectSbomFormat(sbomData);
    // Extract relevant fields for evidence
    const fields = extractSbomFields(sbomData, format);
    const evidence = [
        {
            type: 'file_path',
            value: foundPath,
    ...
    // If format is unknown, it's still a valid JSON but not a recognized SBOM format
    if (format === 'unknown') {
        aggregator.add({
            code: 'SBOM_PRESENT_AND_VALID',
            title: 'SBOM format not recognized',
            severity: 'error',
            details: `SBOM file at ${foundPath} is valid JSON but does not appear to be CycloneDX or SPDX format.`,
            evidence,
            resolution: 'Ensure the SBOM follows CycloneDX (with bomFormat/specVersion) or SPDX (with spdxVersion/SPDXID) format.',
        });
        return;
    }
    // Success case - SBOM found and valid
    // We don't add a violation for success, but we could log the evidence
    // The check passes silently
}
/**
 * Check if SBOM files exist at specified paths
 */
function checkSbom(config, aggregator) {
    if (!config.require_sbom) {
        return;
    }
    const foundPaths = [];
...
/**
 * Extract top-level fields from provenance data for evidence
 */
function extractProvenanceFields(provenanceData) {
    if (typeof provenanceData !== 'object' || provenanceData === null) {
        return '';
    }
    const data = provenanceData;
    const fields = [];
    // Extract common top-level fields from provenance attestations
    // These are typical fields found in SLSA provenance or in-toto attestations
    const topLevelKeys = [
        '_type',
        'predicateType',
        'subject',
...
/**
 * Check if provenance is present and valid JSON
 */
function checkProvenancePresentAndValid(config, aggregator) {
    const missingPaths = [];
    let foundPath = null;
    let provenanceData = null;
    // Check if any provenance file exists
    for (const path of config.provenance_paths) {
        if ((0, fs_1.existsSync)(path)) {
            foundPath = path;
            break;
        }
    ...
    if (!foundPath) {
        // If provenance is required, report as error
        if (config.require_provenance) {
            ...
            aggregator.add({
                code: 'PROVENANCE_PRESENT',
                title: 'Provenance attestation not found',
                severity: 'error',
                details: `No provenance attestation files were found at the configured paths: ${missingPaths.join(', ')}`,
                evidence,
                resolution: 'Generate build provenance using SLSA or similar frameworks and include the attestation file in your repository.',
            });
        }
        // If provenance is not required and not found, do nothing (no benefit to report)
        return;
    }
    // Try to parse JSON
    try {
        const fileContent = (0, fs_1.readFileSync)(foundPath, 'utf-8');
        provenanceData = JSON.parse(fileContent);
    }
    catch (error) {
        const err = error;
        const errorSnippet = err.message.substring(0, 200);
        ...
        if (config.require_provenance) {
            ...
            aggregator.add({
                code: 'PROVENANCE_PRESENT',
                title: 'Provenance file has invalid JSON',
                severity: 'error',
                details: `Provenance file at ${foundPath} contains invalid JSON: ${errorSnippet}`,
                evidence,
                resolution: 'Ensure the provenance file is valid JSON format.',
            });
        }
        else {
            // If provenance is not required but file exists with invalid JSON, report as info
            const evidence = [
                {
                    type: 'file_path',
                    value: foundPath,
            ...
            aggregator.add({
                code: 'PROVENANCE_PRESENT',
                title: 'Provenance file found but has invalid JSON',
                severity: 'info',
                details: `Provenance file at ${foundPath} was found (not required) but contains invalid JSON: ${errorSnippet}`,
                evidence,
                resolution: 'Consider fixing the JSON format to get full benefit from provenance tracking.',
            });
        }
        return;
    }
    // Extract relevant fields for evidence
    const fields = extractProvenanceFields(provenanceData);
    const evidence = [
        {
            type: 'file_path',
            value: foundPath,
    ...
    // If provenance is required, success - no violation added
    // If provenance is not required but found, report as info (benefit without gate)
    if (!config.require_provenance) {
        aggregator.add({
            code: 'PROVENANCE_PRESENT',
            title: 'Provenance attestation found (optional)',
            severity: 'info',
            details: `Provenance attestation file found at ${foundPath}. This provides additional supply chain security even though it's not required.`,
            evidence,
            resolution: 'No action needed. This is informational only.',
        });
    }
}
/**
 * Check if provenance attestation exists
 */
function checkProvenance(config, aggregator) {
    if (!config.require_provenance) {
        return;
    }
    const foundPaths = [];
 ...
 */
function runChecks(config, aggregator) {
    checkSbomPresentAndValid(config, aggregator);
    checkSbom(config, aggregator);
    checkProvenancePresentAndValid(config, aggregator);
}
...
const defaults_1 = __nccwpck_require__(1940);
const schema_json_1 = __importDefault(__nccwpck_require__(5986));
 ...
 * This is the core version without GitHub Actions dependencies
 */
...
/**
 * Core compliance checking logic without GitHub Actions dependencies.
 * This module can be used by both the GitHub Action and CLI.
 */
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.ConfigError = exports.loadConfig = exports.runChecks = exports.ViolationsAggregator = void 0;
exports.shouldFailOnViolations = shouldFailOnViolations;
var violations_1 = __nccwpck_require__(6377);
Object.defineProperty(exports, "ViolationsAggregator", ({ enumerable: true, get: function () { return violations_1.ViolationsAggregator; } }));
var checks_1 = __nccwpck_require__(952);
Object.defineProperty(exports, "runChecks", ({ enumerable: true, get: function () { return checks_1.runChecks; } }));
var config_loader_1 = __nccwpck_require__(4743);
Object.defineProperty(exports, "loadConfig", ({ enumerable: true, get: function () { return config_loader_1.loadConfig; } }));
Object.defineProperty(exports, "ConfigError", ({ enumerable: true, get: function () { return config_loader_1.ConfigError; } }));
/**
 * Helper function to determine if the mode should fail on violations
 */
function shouldFailOnViolations(mode) {
    // Both 'fail' and 'enforce' modes cause the job to fail on violations
    return mode === 'fail' || mode === 'enforce';
}
//# sourceMappingURL=index.js.map
...
/**
 * Violation types and aggregator for evidence gate checks
 */
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.ViolationsAggregator = void 0;
class ViolationsAggregator {
    constructor() {
        this.violations = [];
    }
    add(violation) {
...
exports.ViolationsAggregator = ViolationsAggregator;
...
module.exports = /*#__PURE__*/JSON.parse('{"$schema":"http://json-schema.org/draft-07/schema#","type":"object","properties":{"mode":{"type":"string","enum":["warn","fail","enforce"],"description":"Action mode: \'warn\' logs warnings, \'fail\' fails the workflow, \'enforce\' strictly enforces all rules"},"require_sbom":{"type":"boolean","description":"Whether to require SBOM files"},"require_provenance":{"type":"boolean","description":"Whether to require provenance attestations"},"sbom_paths":{"type":"array","items":{"type":"string"},"minItems":1,"description":"List of paths to search for SBOM files"},"provenance_paths":{"type":"array","items":{"type":"string"},"minItems":1,"description":"List of paths to search for provenance attestation files"}},"additionalProperties":false}');
...
/**
 * CLI for Evidence Gate - Run compliance checks locally
 * Usage: evidence-gate [--config-path <path>]
 */
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core_1 = __nccwpck_require__(2185);
/**
...
function getSeverityDisplay(severity) {
    switch (severity) {
        ...
        case 'warning':
            return { color: colors.yellow, emoji: '⚠️' };
        case 'info':
            return { color: colors.cyan, emoji: 'ℹ️' };
        default:
            return { color: colors.reset, emoji: '•' };
    }
}
/**
 * Get status display based on mode and violations
 */
function getStatusDisplay(aggregator, mode) {
    if (aggregator.isEmpty()) {
        return { status: 'PASS', color: colors.green, emoji: '✅' };
    }
    const summary = aggregator.getSummary();
    if (summary.errors > 0) {
        if (mode === 'warn') {
            return { status: 'WARN', color: colors.yellow, emoji: '⚠️' };
        }
        return { status: 'FAIL', color: colors.red, emoji: '❌' };
    }
    return { status: 'PASS', color: colors.green, emoji: '✅' };
}
/**
...
function parseArgs() {
    ...
    let configPath = '.compliance-gate.yml';
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--config-path' && i + 1 < args.length) {
            ...
            console.log(`
${colors.bright}Evidence Gate CLI${colors.reset}

Usage: evidence-gate [options]

Options:
  --config-path <path>  Path to configuration file (default: .compliance-gate.yml)
  --help, -h           Show this help message

Examples:
  evidence-gate
  evidence-gate --config-path .github/compliance.yml
`);
            process.exit(0);
        }
    }
...
async function main() {
    try {
        console.log(`${colors.bright}${colors.cyan}🚀 Evidence Gate - Compliance Checks${colors.reset}\n`);
        // Parse arguments
        ...
        console.log(`   - Require SBOM: ${config.require_sbom}`);
        console.log(`   - Require Provenance: ${config.require_provenance}`);
        console.log(`   - SBOM Paths: ${config.sbom_paths.join(', ')}`);
        console.log();
        // Create violations aggregator
        const aggregator = new core_1.ViolationsAggregator();
        // Run compliance checks
        console.log(`${colors.bright}Running compliance checks...${colors.reset}`);
        (0, core_1.runChecks)(config, aggregator);
        // Get summary
        const summary = aggregator.getSummary();
        const statusDisplay = getStatusDisplay(aggregator, config.mode);
        console.log();
        console.log(`${colors.bright}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}`);
        console.log(`${colors.bright}${statusDisplay.color}${statusDisplay.emoji} Status: ${statusDisplay.status}${colors.reset}`);
        console.log(`${colors.bright}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}`);
        console.log();
        // Print summary
        ...
        console.log(`   ${colors.cyan}ℹ️  Info:${colors.reset}        ${summary.info}`);
        console.log();
        // Print violations details
        if (!aggregator.isEmpty()) {
            console.log(`${colors.bright}🔍 Violation Details:${colors.reset}`);
            console.log();
            const violations = aggregator.getViolations();
            for (const violation of violations) {
                const display = getSeverityDisplay(violation.severity);
                console.log(`${display.color}${display.emoji} ${colors.bright}${violation.title}${colors.reset}`);
                ...
                if (violation.evidence && violation.evidence.length > 0) {
                    ...
                    for (const evidence of violation.evidence) {
                        if (evidence.type === 'missing_file') {
                            console.log(`     - Missing file: ${evidence.value}`);
                        }
                        else if (evidence.type === 'file_path') {
                            console.log(`     - File: ${evidence.value}`);
                        }
                        else if (evidence.type === 'format') {
                            console.log(`     - Format: ${evidence.value}`);
                        }
                        else if (evidence.type === 'fields') {
                            console.log(`     - Fields: ${evidence.value}`);
                        }
                        else if (evidence.type === 'parse_error') {
                            const errorMsg = evidence.value.substring(0, 100);
                ...
                if (violation.resolution) {
                    console.log(`   ${colors.green}✅ Resolution:${colors.reset} ${violation.resolution}`);
                }
                console.log();
            }
        }
        ...
        let exitCode = 0;
        if ((0, core_1.shouldFailOnViolations)(config.mode) && aggregator.hasErrors()) {
            exitCode = 1;
            console.log(`${colors.red}${colors.bright}❌ Compliance check FAILED${colors.reset}`);
            console.log(`   Found ${summary.errors} error(s) in ${config.mode} mode`);
        }
        else if (config.mode === 'warn' && aggregator.hasErrors()) {
            exitCode = 0;
            console.log(`${colors.yellow}${colors.bright}⚠️  Compliance check completed with warnings${colors.reset}`);
            console.log(`   Found ${summary.errors} error(s), but mode is 'warn'`);
        }
        else {
        ...
        if (error instanceof core_1.ConfigError) {
            console.error(`${colors.red}${colors.bright}❌ Configuration Error${colors.reset}`);
            console.error();
